...
namespace CompleteProject
{
	public class PlayerMovement : MonoBehaviour
    {
        ...
        void Move (float h, float v)
        {
            ...
            var playerpos = transform.position + movement;

            var cameraPos = camera.gameObject.transform.position;

            var cameraSize = 15f;

            var x1 = cameraPos.x - cameraSize / 2;

            var x2 = cameraPos.x + cameraSize / 2;

            var z1 = cameraPos.z - cameraSize;

            var z2 = cameraPos.z + cameraSize;

            float PlayerX = Mathf.Clamp(playerpos.x, x1, x2);

            float PlayerZ = Mathf.Clamp(playerpos.z, z1, z2);

            Vector3 newPos = new Vector3(PlayerX, 0, PlayerZ);
        ...
        void Turning ()
        {
            ...
            else if (GameManager.instance.runningMac)
            {
